...
if(isset($_POST['productId']) && isset($_SESSION['user_id']) && isLogin()){
  ...
  try {
    ...
    if(!empty($resultCount)){
      // レコードを削除する
      $sql = 'DELETE FROM good WHERE product_id = :p_id AND user_id = :u_id';
      $data = array(':u_id' => $_SESSION['user_id'], ':p_id' => $p_id);
      // クエリ実行
      $stmt = queryPost($dbh, $sql, $data);
      debug('いいねの数：'.count(getGood($p_id)));
			echo count(getGood($p_id));
    }else{
      // レコードを挿入する
      $sql = 'INSERT INTO good (product_id, user_id, create_date) VALUES (:p_id, :u_id, :date)';
